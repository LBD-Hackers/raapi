...
exports.findReferenceRegistry = findReferenceRegistry;

const {
  QueryEngine
} = require('@comunica/query-sparql');

const {
  getSatelliteFromLdpResource,
  CONSOLID
} = require('consolid-daapi');
...
async function findReferenceRegistry(projectUrl) {
  ...
  if (bindings.length) return bindings[0].get('refReg').value;else throw new Error('could not find reference registry for this project');
}
