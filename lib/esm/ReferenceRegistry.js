...
class ReferenceRegistry {
  ...
  async create(parent, makePublic = true) {
    ...
    await metadata.create(makePublic, [{
      ...
      object: _consolidDaapi.CONSOLID.ReferenceRegistry
    }]);
  ...
  async createConcept(concept = this.url + "#" + (0, _uuid.v4)()) {
    let query = `INSERT DATA {
      <${concept}> a <${_consolidDaapi.CONSOLID.Concept}> .
    }`;
  ...
  async registerAggregatedConcept(concept, aggregatedConcept) {
    let query = `INSERT DATA {
      <${concept}> <${_consolidDaapi.CONSOLID.aggregates}> <${aggregatedConcept}> .
    }`;
    await this.update(query);
  }
  ...
  async createReference(concept, reference = (0, _consolidDaapi.getRoot)(this.url) + (0, _uuid.v4)(), document, identifier) {
    ...
    const now = new Date();
    let query = `INSERT DATA {
      <${concept}> <${_consolidDaapi.CONSOLID.aggregates}> <${reference}> .
      <${reference}> <${_vocabCommonRdf.DCTERMS.created}> "${now}";
        <${_consolidDaapi.CONSOLID.hasIdentifier}> <${id}> .
      <${id}> <https://schema.org/value> "${identifier}" ;
        <${_consolidDaapi.CONSOLID.inDocument}> <${document}> .
    }`;
    await this.update(query);
    return reference;
  }
  ...
  async findConceptByIdentifier(activeDocument, selectedElement) {
    ...
    for (const sat of endpoints) {
      const q = `SELECT * WHERE {
            ...
            ?ds a <${_consolidDaapi.CONSOLID.ReferenceRegistry}> ;
            <${_vocabCommonRdf.DCAT.distribution}>/<${_vocabCommonRdf.DCAT.downloadURL}> ?ref .
            GRAPH ?ref {
                ?concept a <${_consolidDaapi.CONSOLID.Concept}> ;
                    <${_consolidDaapi.CONSOLID.aggregates}> ?reference .
                ?reference <${_consolidDaapi.CONSOLID.hasIdentifier}> ?id .
                ?id <${_consolidDaapi.CONSOLID.inDocument}> <${activeDocument}>;
                    <https://schema.org/value> "${selectedElement}".
                    OPTIONAL {?concept <${_consolidDaapi.CONSOLID.aggregates}> ?aggr .
    ...
    for (const alias of aliases) {
      ...
      const q = `SELECT * WHERE {
            <${alias}> a <${_consolidDaapi.CONSOLID.Concept}> ;
                    <${_consolidDaapi.CONSOLID.aggregates}> ?reference .
                ?reference <${_consolidDaapi.CONSOLID.hasIdentifier}> ?id .
                ?id <${_consolidDaapi.CONSOLID.inDocument}> ?doc;
